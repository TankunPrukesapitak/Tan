...
describe('Register Feature', () => {
  ...
  it('TC_R_1 Verify that the user is able to register successfully with the correct email, password and confirm password.', () => {
    registerKeyword.registerWithCredentials(registerTestData.randomEmail, registerTestData.password, registerTestData.confirmpassword);
    ...
    cy.url().should('eq', `${locator.BaseURL}${locator.PathURL_2}`);
    cy.get('[id="SignIn"]').should('be.visible');
  })

  it('TC_L_2 Verify that the user cannot register successfully if using a duplicate email.', () => {
    registerKeyword.registerWithCredentials(loginTestData.validEmail, registerTestData.password, registerTestData.confirmpassword);
    cy.get(locator.registerButton).click();
  ...
  it('TC_L_3 Verify that the user cannot register successfully if the password and password confirmation do not match.', () => {
    registerKeyword.registerWithCredentials(registerTestData.randomEmail, registerTestData.password, registerTestData.confirmpassword_2);
    cy.get(locator.registerButton).click();
